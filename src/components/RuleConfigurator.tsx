...
import { Rule, NaturalLanguageRule, GeminiRuleConversionResponse, CoRunRule, SlotRestrictionRule, LoadLimitRule, PhaseWindowRule, PatternMatchRule, PrecedenceOverrideRule } from '@/types';
...
import {
  FiPlusCircle, FiZap, FiTrash2, FiEdit3, FiCheckCircle, FiXCircle,
  FiLoader, FiToggleLeft, FiToggleRight, FiInfo, FiDownload, FiChevronDown, FiChevronUp, FiCopy
} from 'react-icons/fi';
...
const getInitialNewRule = (): Omit<Rule, 'id' | 'source' | 'description' | 'type'> & { type: string | ''; description: string } => ({
  type: '',
  description: '',
  isEnabled: true,
});
...
const RuleConfigurator: React.FC = () => {
  ...
  const visibleRules = useMemo(() => rules.filter(rule => rule.type !== 'precedenceOverride'), [rules]);
  const currentPrecedenceRule = useMemo(() => rules.find(rule => rule.type === 'precedenceOverride') as PrecedenceOverrideRule | undefined, [rules]);
  ...
  const copyToClipboard = useCallback((text: string, message: string) => {
    if (navigator.clipboard && navigator.clipboard.writeText) {
      ...
      toast.error("Copy failed: Your browser does not support clipboard access.");
    }
  }, []);
  ...
  const handleSaveRule = useCallback(() => {
    ...
    setRules(prevRules => {
      if (editingRule) {
        return prevRules.map(rule => rule.id === newOrUpdatedRule.id ? newOrUpdatedRule : rule);
      } else {
    ...
    setEditingRule(null);
    setFormRule(getInitialNewRule());
    toast.success(editingRule ? "Rule updated successfully!" : "New rule created!");
  ...
  const handleConvertNlToRule = useCallback(async () => {
    ...
    try {
      ...
      if (response.ok && result.success && result.suggestedRule) {
        ...
        const errorMessage = result.message || 'Failed to convert rule from natural language.';
  ...
  const onDragEnd = useCallback((result: DropResult) => {
    ...
    const newPrecedenceRule: PrecedenceOverrideRule = {
      ...
      description: 'Defines the custom execution order of rules via drag-and-drop.',
      isEnabled: true,
      source: 'manual',
        ...
        return (
          <div className="space-y-2">
            ...
            <Input
              ...
              onChange={(e) => handleFormChange('taskIds', e.target.value.split(',').map(s => s.trim()).filter(s => s !== ''))}
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="space-y-2 col-span-2">
              ...
              <Input
                id="minCommonSlots"
                type="number"
                value={typeof (formRule as SlotRestrictionRule).minCommonSlots === 'number' ? (formRule as SlotRestrictionRule).minCommonSlots : ''}
                onChange={(e) => handleFormChange('minCommonSlots', parseInt(e.target.value) || 0)}
                placeholder="e.g., 5"
              />
            </div>
             <div className="space-y-2 col-span-2">
              <Label htmlFor="targetPhases">Target Phases (comma-separated numbers)</Label>
              <Input
                id="targetPhases"
                value={(formRule as SlotRestrictionRule).targetPhases?.join(',') || ''}
                onChange={(e) => handleFormChange('targetPhases', e.target.value.split(',').map(s => parseInt(s.trim())).filter(n => !isNaN(n)))}
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) => handleFormChange('workerGroup', e.target.value)}
            ...
            <div className="space-y-2">
              ...
              <Input
                id="maxLoad"
                type="number"
                value={typeof (formRule as LoadLimitRule).maxLoad === 'number' ? (formRule as LoadLimitRule).maxLoad : ''}
                onChange={(e) => handleFormChange('maxLoad', parseInt(e.target.value) || 0)}
                placeholder="e.g., 10"
              />
            </div>
            <div className="space-y-2 col-span-2">
              ...
              <Input
                id="phase"
                type="number"
                value={typeof (formRule as LoadLimitRule).phase === 'number' ? (formRule as LoadLimitRule).phase : ''}
                onChange={(e) => {
        ...
        return (
          <div className="space-y-2">
            ...
            <Input
              ...
              placeholder="e.g., T5"
            />
            <Label htmlFor="allowedPhases">Allowed Phases (comma-separated: numbers or ranges like 1-3)</Label>
            <Input
              id="allowedPhases"
              value={(formRule as PhaseWindowRule).allowedPhases?.join(',') || ''}
              onChange={(e) => handleFormChange('allowedPhases', e.target.value.split(',').map(s => s.trim()).filter(s => s !== ''))}
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            {(formRule as PatternMatchRule).action === 'transform' && (
              <div className="space-y-2">
                ...
                <Input
                  id="transformTo"
                  value={(formRule as PatternMatchRule).actionDetails?.transformTo || ''}
                  onChange={(e) => handleFormChange('actionDetails', { ...formRule.actionDetails, transformTo: e.target.value })}
            ...
            {(formRule as PatternMatchRule).action === 'flag' && (
              <div className="space-y-2">
                ...
                <Input
                  id="message"
                  value={(formRule as PatternMatchRule).actionDetails?.message || ''}
                  onChange={(e) => handleFormChange('actionDetails', { ...formRule.actionDetails, message: e.target.value })}
        ...
        return (
          <div className="space-y-2">
            <p className="text-muted-foreground text-sm flex items-center">
                <FiInfo className="mr-1 text-primary" /> This rule is managed in the 'Rule Prioritization (Drag & Drop)' section below.
            </p>
          </div>
        );
      default:
  ...
  return (
    <Card className="mt-8 p-6 md:p-8">
      <CardHeader className="text-center">
        <CardTitle className="text-3xl font-bold text-foreground">Configure Business Rules</CardTitle>
        <CardDescription className="text-muted-foreground mt-2">
          Define how your data should be processed and resources allocated, or let AI assist.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-8">
        ...
        <div className="space-y-4 p-4 border border-border rounded-lg bg-muted/20">
          ...
          <p className="text-sm text-muted-foreground">Type your rule in plain English and let AI convert it to a structured format.</p>
          <Textarea
        ...
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          <DialogTrigger asChild>
            <Button
              ...
              className="w-full"
            >
              <FiPlusCircle className="mr-2 h-4 w-4" /> Create New Rule Manually
            </Button>
          </DialogTrigger>
          <DialogContent className="sm:max-w-[600px] max-h-[80vh] overflow-y-auto">
            <DialogHeader>
              ...
              <DialogDescription>
                {editingRule ? 'Modify the properties of this rule.' : 'Define a new business rule manually.'}
              </DialogDescription>
            </DialogHeader>
            <div className="grid gap-4 py-4">
              <div className="space-y-2">
                ...
                <Select
                  ...
                  disabled={!!editingRule && (editingRule.type === 'naturalLanguage')}
                >
                 ...
                 {(editingRule && editingRule.type === 'naturalLanguage') && (
                  <p className="text-xs text-muted-foreground mt-1 flex items-center">
                    <FiInfo className="mr-1 text-primary" /> Type cannot be changed for AI-generated rules.
                  </p>
                )}
              </div>
        ...
        <div className="space-y-4 p-4 border border-border rounded-lg bg-muted/20">
          <h3 className="text-xl font-semibold text-foreground flex items-center mb-4">
            <FiCheckCircle className="mr-2 text-primary" /> Rule Prioritization (Drag & Drop)
          </h3>
          <p className="text-sm text-muted-foreground mb-4">Drag and drop rules below to define their execution order. Rules are applied from top to bottom.</p>
                        ...
                        {(provided, snapshot) => (
                          <div
                            ...
                            className={`p-3 bg-secondary rounded-md border border-border flex items-center justify-between text-foreground
                                        ${snapshot.isDragging ? 'shadow-lg bg-primary/20 scale-102' : ''} transition-all`}
                          >
                            <span className="font-medium text-sm flex-1 truncate">
                              {index + 1}. {rule.description}
                              <code className="font-mono text-xs ml-2 text-muted-foreground/80">(ID: {rule.id})</code>
                            </span>
                            <div className="flex items-center space-x-2">
                               {rule.isEnabled ? (
                                   <span className="text-emerald-500 text-xs ml-1 flex items-center font-medium"><FiCheckCircle className="inline-block h-3 w-3 mr-1" /> Active</span>
                               ) : (
                                   <span className="text-rose-500 text-xs ml-1 flex items-center font-medium"><FiXCircle className="inline-block h-3 w-3 mr-1" /> Inactive</span>
                               )}
                               <Button variant="ghost" size="icon" className="h-6 w-6 ml-1" onClick={() => copyToClipboard(rule.id, 'Rule ID copied!')} title="Copy Rule ID">
          ...
          {currentPrecedenceRule && currentPrecedenceRule.ruleIds.length > 0 && (
            <div className="mt-3 p-3 bg-card/50 border border-border rounded-md text-sm text-foreground">
              <p className="font-medium text-primary-foreground/80">Current Active Order (for export):</p>
        ...
        <div className="text-center">
          <Button onClick={exportRulesToJson} className="w-full">
        ...
        <div className="space-y-4">
          <h3 className="text-xl font-semibold text-foreground flex items-center">
            <FiPlusCircle className="mr-2 text-primary" /> All Configured Rules ({visibleRules.length})
          </h3>
          {visibleRules.length === 0 ? (
            <p className="text-muted-foreground text-center p-4 border border-border rounded-lg">
              No rules configured yet. Start by typing a natural language rule or create one manually!
            </p>
          ) : (
            <div className="space-y-4">
              {visibleRules.map((rule, index) => (
                <Card key={rule.id} className="p-4 bg-secondary/20 border border-border">
                  <div className="flex justify-between items-start mb-2">
                    <div>
                      <h4 className="font-semibold text-foreground text-lg mb-1">
                        {rule.type === 'naturalLanguage' ? 'AI Generated Rule' : ruleTypeOptions.find(opt => opt.value === rule.type)?.label || rule.type.replace(/([A-Z])/g, ' $1').trim()}
                      </h4>
                      <p className="text-muted-foreground text-sm flex items-center">
                         ID: <code className="font-mono text-xs ml-1 dark:text-white text-black">{rule.id}</code>
                         <Button
                            variant="ghost"
                            size="icon"
                            className="h-6 w-6 ml-1"
                            onClick={() => copyToClipboard(rule.id, 'Rule ID copied!')}
                            title="Copy Rule ID"
                         >
                            <FiCopy className="h-3 w-3 text-muted-foreground" />
                            <span className="sr-only">Copy Rule ID</span>
                      ...
                      <p className="text-muted-foreground text-sm flex items-center mt-1">
                         Status: {rule.isEnabled ? (
                           <span className="text-primary ml-1 flex items-center font-medium"><FiCheckCircle className="inline-block h-3 w-3 mr-1" /> Active</span>
                         ) : (
                           <span className="text-destructive ml-1 flex items-center font-medium"><FiXCircle className="inline-block h-3 w-3 mr-1" /> Inactive</span>
                         )}
                      </p>
                    </div>
                    <div className="flex items-center space-x-2">
                      <Button
                        ...
                        size="icon"
                        className="h-8 w-8"
                        onClick={() => handleToggleEnable(rule.id, !rule.isEnabled)}
                        title={rule.isEnabled ? 'Deactivate Rule' : 'Activate Rule'}
                      >
                          {rule.isEnabled ? (
                          ...
                          <span className="sr-only">{rule.isEnabled ? 'Deactivate rule' : 'Activate rule'}</span>
                      </Button>
                      <Button
                        variant="ghost"
                        size="icon"
                  ...
                  <p className="text-foreground text-base mb-2">{rule.description}</p>
                  <div className="mt-3">
                    <Button
                      variant="ghost"
                      size="sm"
                    ...
                    >
                      {expandedRules.has(rule.id) ? (
                        <>
                          <FiChevronUp className="mr-1 h-4 w-4" /> Collapse Details
                        </>
                      ) : (
                        <>
                          <FiChevronDown className="mr-1 h-4 w-4" /> View Details
                        </>
                      )}
                    </Button>
                    {expandedRules.has(rule.id) && (
                      <div className="p-3 bg-card/50 border border-border rounded-md text-sm text-foreground mt-2">
                          <p className="font-medium text-primary-foreground/80 mb-2">Structured Rule Details:</p>
                          <pre className="whitespace-pre-wrap break-words text-xs text-muted-foreground">
                              {JSON.stringify(
                                  rule.type === 'naturalLanguage' ? (rule as NaturalLanguageRule).suggestedStructuredRule : rule,
                                  null,
                                  2
                              )}
                          </pre>
                          {rule.type === 'naturalLanguage' && (rule as NaturalLanguageRule).aiConfidence && <p className="text-xs text-muted-foreground mt-1">Confidence: {((rule as NaturalLanguageRule).aiConfidence! * 100).toFixed(0)}%</p>}
                      </div>
                    )}
                  </div>
                </Card>
